...
<div align="center">
    <c:if test="${requestScope.ruo != null}">
    <form action="Ruoli" method="post">
        <input type="hidden" name="idRuo" value="${requestScope.ruo.id}">
        </c:if>
        <c:if test="${requestScope.ruo == null}">
        <form action="Ruoli" method="post">
            </c:if>
            <table border="1" cellpadding="5">
                ...
                <c:if test="${requestScope.ruo != null}">
                    <input type="hidden" name="id" value="<c:out value='${ruo.getId()}' />" />
                </c:if>
...
<%--                    modifica ruolo--%>
                    <tr>
                        ...
                        <td>
                            <input class="form-control" type="text" name="dettagli" size="45"
                                   value="<c:out value='${ruo.getDettagli()}' />" />
                        </td>
                    </tr>
                </c:if>
                <tr>
                    ...
                    <td>
                        <select class="form-control" name="stato">
                            ...
                            <option value="attivo"
                                    <c:if test="${requestScope.ruo.getStato() == 'attivo'}">
                                        selected="selected"
                                    </c:if>
                            >Attiva</option>
                            <option value="nonat"
                                    <c:if test="${requestScope.ruo.getStato() == 'nonat'}">
                                        selected="selected"
<%--  servono le tonde solo quando prendo qualcosa dalla servlet--%>
                                    </c:if>>Disabilita</option>
                        </select>
                    </td>
                </tr>
                <tr>
